...
export async function implementItem(deps: ImplementDeps, item: PlanItem, body: string): Promise<ImplementOutcome> {
	...
	try {
		...
		// This is the one progress line the phase keeps: sandcastle knows nothing
 ...
 *
 * A line per item, and nothing more on a green one — its log file is already on
 * screen from sandcastle's own startup line. A red item is the exception:
 * everything it takes to know why hangs under it.
 */
export function renderOutcomes(outcomes: readonly ImplementOutcome[]): string {
	const lines: string[] = [];

	for (const outcome of outcomes) {
		const mark = outcome.status === 'implemented' ? '✓' : '✗';
		const gate = outcome.gate
			? `, gate ${outcome.gate.passed ? 'passed' : `failed, exit ${outcome.gate.exitCode}`}`
			: '';

		lines.push(
			`  ${mark} #${outcome.issue} ${outcome.branch} — ${outcome.status}, ` +
				`${outcome.commits} commit(s) over ${outcome.iterations} iteration(s)${gate}`
		);
		...
		if (outcome.gate && !outcome.gate.passed && outcome.gate.tail.trim() !== '') {
		...
		// Green items had their log path printed when the run started; a red one
		// is worth repeating it for, at the point the operator goes looking.
		if (outcome.status !== 'implemented' && outcome.logFilePath) lines.push(`      log: ${outcome.logFilePath}`);
	}
