...
/**
 * One line per item: where it is cut from and where it ends up are the two
 * facts a plan exists to state, and both fit on the line with the branch.
 */
export function renderPlan(plan: Plan): string {
	const lines: string[] = [`Plan — validated: ${plan.items.length} item(s) on ${plan.baseBranch}.`];

	for (const item of plan.items) {
		// Only a child has somewhere to merge into; saying "PR only" on every
		// other line is a column of noise for the ordinary case.
		const target = item.mergeInto === null ? '' : `, merges into ${item.mergeInto}`;
		lines.push(`  #${item.issue} ${item.title} → ${item.workBranch} (${item.role}) from ${item.base}${target}`);
	}

	for (const deferred of plan.deferred) {
		lines.push(`  deferred #${deferred.issue} ${deferred.title} — ${deferred.reason}`);
	}
