...
/** One line per child: where its work went, and whether the issue is done. */
export function renderMergeOutcomes(outcomes: readonly MergeOutcome[]): string {
	const lines: string[] = [];

	for (const outcome of outcomes) {
		const mark = isMergeFailure(outcome) ? '✗' : '✓';
		const commit = outcome.sha ? ` as ${outcome.sha.slice(0, 7)}` : '';

		lines.push(
			`  ${mark} #${outcome.issue} ${outcome.branch} → ${outcome.epic}${commit} — ` +
				`${outcome.status}, issue ${outcome.closed ? 'closed' : 'still open'}`
		);
