...
import { nodeVersionRefusal } from './lib/node.mts';
...
async function main(): Promise<void> {
	// Before anything imports sandcastle: on an old runtime that import dies
	// inside node_modules with a `styleText` error that names nothing useful.
	const tooOld = nodeVersionRefusal(process.version);
	if (tooOld) {
		console.error(`error: ${tooOld}`);
		process.exit(2);
	}
	...
	console.log(
		dryRun
			? `Pre-flight: on ${started.startedOn}; nothing healed — a dry run touches nothing at all.`
	...
	ensureSandboxImage(realImageDeps(cwd, (message) => console.log(`Sandbox image: ${message}`)), SANDBOX_IMAGE);
	...
	const ready = freshen(cwd, first, 'Branches');
	const outcomes = await implement(cwd, ready, result.eligible);

	const readyChildren = children.length > 0 ? freshen(cwd, children, 'Child branches') : [];
...
function prepareEpicBranches(
	...
	// The messages already name themselves ("#29: created and linked epic
	// branch …"), so they need no section header above them.
 ...
 * stale base.
 *
 * A branch cut fresh from its base is the ordinary case and says nothing an
 * operator needs; the counts cover it. Anything else — carried work, a skip —
...
function freshen(cwd: string, items: readonly PlanItem[], label: string): PlanItem[] {
	...
	for (const item of items) {
		...
		if (outcome.action !== 'created') notable.push(`  ${item.workBranch}: ${describeFreshness(outcome)}`);
		if (outcome.action !== 'skipped') ready.push(item);
	}

	console.log(`\n${label}: ${ready.length} of ${items.length} at a correct starting point.`);
 ...
 *
 * Silent on the way through: each wave already printed its own outcomes, and
 * saying the same count a third time is not a verdict, it is noise.
 */
function reportImplemented(outcomes: readonly ImplementOutcome[]): void {
	if (outcomes.length === 0) return;

	if (!outcomes.some((outcome) => outcome.status === 'implemented')) process.exitCode = 1;
}
...
function merge(
	...
	// The per-child progress goes to the same place the outcomes do, so only the
	// outcomes are printed: they carry every fact the progress lines had.
	const results = mergeChildren(realMergeDeps(cwd, repo, () => {}), children);
...
async function implement(
	...
	console.log(`\nImplemented ${done.length} of ${outcomes.length} item(s) with the gate green:\n${renderOutcomes(outcomes)}`);
...
function finalize(
	...
	// Same as the merges: the handoff block below says everything the push and
	// PR progress lines would have. The PR-body agent's fallbacks are the
	// exception — nothing downstream records those — so they stay on `log`.
	...
	console.log(
		`\nFinalized ${finalizable.length} of ${outcomes.length} item(s) — ` +
