...
menu save, scheduled purges, the log file, the REST API, the French translation,
...
- [ ] **Settings → Next.js revalidate.** Expect six tabs — **Next.js API**,
      **Allow purge all**, **On menu update**, **Debug**, **Queue**, **Probe** —
      with a count badge on Queue. Click each: expect one panel visible at a
      time, Probe included, though it is a form of its own. (All six stacked
      means a broken `settings.js`.)
