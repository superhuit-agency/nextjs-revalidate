<?php
/**
 * The endpoint URL — Settings::endpoint_url().
 *
...
const FSE_PATH = Settings::LEGACY_FSE_ENDPOINT_PATH_NAME;
...
site( [ DOMAIN => 'https://front-end.test', PATH_OPT => '/fr/api/purge' ] );
check_same( 'https://front-end.test/fr/api/purge', $settings->endpoint_url(), 'a custom revalidate path is used verbatim' );

// There is one endpoint from v2 (ADR 0034). A site still holding the FSE
// snapshot's own path, until the upgrade deletes it, sends nothing there.
site( [ DOMAIN => 'https://front-end.test', FSE_PATH => '/api/fse' ] );
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'a leftover FSE path does not disturb the one endpoint' );
...
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'a trailing slash on the domain does not double up' );

site( [ DOMAIN => 'https://front-end.test', PATH_OPT => 'api/revalidate' ] );
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'a path without a leading slash still joins with one' );
...
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'a path of "/" falls back to the default' );

// #65: a site can hold a row stored as false, which reads as the empty string.
site( [ DOMAIN => 'https://front-end.test', PATH_OPT => false ] );
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'a path stored as false falls back to the default' );
...
check_same( 'http://localhost:8083/revalidate', $settings->endpoint_url(), 'a port is carried through' );

site( [ DOMAIN => 'https://front-end.test/blog', PATH_OPT => '/api/revalidate' ] );
check_same( 'https://front-end.test/blog/api/revalidate', $settings->endpoint_url(), 'a subdirectory domain is carried through' );
...
site( [ DOMAIN => '  https://front-end.test  ' ] );
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'whitespace around the domain is trimmed off' );

site( [ DOMAIN => 'https://front-end.test', PATH_OPT => ' /api/purge ' ] );
check_same( 'https://front-end.test/api/purge', $settings->endpoint_url(), 'whitespace around the path is trimmed off' );

// Trimmed before the fallback, so a field an operator left as spaces is a field
// they left empty, not a path of its own.
site( [ DOMAIN => 'https://front-end.test', PATH_OPT => '   ' ] );
check_same( 'https://front-end.test/api/revalidate', $settings->endpoint_url(), 'a path of nothing but whitespace falls back to the default' );

site( [ DOMAIN => '   ' ] );
check_same( '', $settings->endpoint_url(), 'a domain of nothing but whitespace composes nothing' );
...
site( [] );
check_same( '', $settings->endpoint_url(), 'no domain composes no endpoint URL' );

site( [ DOMAIN => '', PATH_OPT => '/api/revalidate' ] );
check_same( '', $settings->endpoint_url(), 'a path without a domain composes nothing' );
