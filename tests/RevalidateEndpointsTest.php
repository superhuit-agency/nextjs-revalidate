<?php
/**
 * The revalidate endpoints — Settings::revalidate_url() and ::fse_revalidate_url().
 *
...
const FSE_PATH = Settings::SETTINGS_FSE_PATH_NAME;
...
check_same( 'https://front-end.test/fr/api/purge', $settings->revalidate_url(), 'a custom revalidate path is used verbatim' );
check_same( 'https://front-end.test/fr/api/purge-fse', $settings->fse_revalidate_url(), 'a custom FSE path is used verbatim' );
...
check_same( 'https://front-end.test/api/revalidate', $settings->revalidate_url(), 'an FSE path does not disturb the revalidate path' );
check_same( 'https://front-end.test/api/fse', $settings->fse_revalidate_url(), 'the FSE path is overridden on its own' );
...
check_same( 'https://front-end.test/api/revalidate', $settings->revalidate_url(), 'a trailing slash on the domain does not double up' );

site( [ DOMAIN => 'https://front-end.test', PATH_OPT => 'api/revalidate' ] );
check_same( 'https://front-end.test/api/revalidate', $settings->revalidate_url(), 'a path without a leading slash still joins with one' );

site( [ DOMAIN => 'https://front-end.test/', PATH_OPT => 'api/revalidate' ] );
check_same( 'https://front-end.test/api/revalidate', $settings->revalidate_url(), 'neither half carrying a slash still joins with one' );
...
check_same( 'https://front-end.test/api/revalidate', $settings->revalidate_url(), 'a path of "/" falls back to the default' );

// #65: a site can hold a row stored as false, which reads as the empty string.
site( [ DOMAIN => 'https://front-end.test', PATH_OPT => false ] );
check_same( 'https://front-end.test/api/revalidate', $settings->revalidate_url(), 'a path stored as false falls back to the default' );
...
check_same( 'http://localhost:8083/revalidate', $settings->revalidate_url(), 'a port is carried through' );

site( [ DOMAIN => 'https://front-end.test/blog', PATH_OPT => '/api/revalidate' ] );
check_same( 'https://front-end.test/blog/api/revalidate', $settings->revalidate_url(), 'a subdirectory domain is carried through' );
...
site( [] );
check_same( '', $settings->revalidate_url(), 'no domain composes no revalidate URL' );
check_same( '', $settings->fse_revalidate_url(), 'no domain composes no FSE URL' );

site( [ DOMAIN => '', PATH_OPT => '/api/revalidate' ] );
check_same( '', $settings->revalidate_url(), 'a path without a domain composes nothing' );
